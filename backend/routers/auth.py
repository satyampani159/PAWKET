...
def get_user_from_token(token, db):
    s = db.query(SessionModel).filter(
        ...
        SessionModel.expires_at > datetime.utcnow()).first()
    if not s:
        return None
    return db.query(User).filter(User.id==s.user_id).first()

def require_auth(authorization: str = Header(...), db: Session = Depends(get_db)):
...
def request_otp(req: OTPRequest, db: Session = Depends(get_db)):
    ...
    db.query(OTPRecord).filter(OTPRecord.phone==phone, OTPRecord.is_used==False).update({"is_used": True})
    db.commit()
    otp = generate_otp()
    db.add(OTPRecord(phone=phone, otp=otp, created_at=datetime.utcnow(),
        expires_at=datetime.utcnow()+timedelta(minutes=OTP_EXPIRY_MINUTES), is_used=False))
    db.commit()
    send_otp_sms(phone, otp)
    return AuthResponse(success=True, message=f"OTP sent. [DEV: {otp}]")

@router.post("/verify-otp", response_model=AuthResponse)
def verify_otp(req: OTPVerify, db: Session = Depends(get_db)):
    phone = req.phone.strip()
    record = db.query(OTPRecord).filter(
        OTPRecord.phone==phone, OTPRecord.otp==req.otp,
        OTPRecord.is_used==False, OTPRecord.expires_at > datetime.utcnow()).first()
    if not record:
    ...
    return AuthResponse(success=True, user_id=user.id, phone=user.phone,
